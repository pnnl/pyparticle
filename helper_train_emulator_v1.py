...
# # =============================================================================
#     model = bambi.Model(
#         'Edry_minus1 ~ np.log(1 / (1 + Rbc)) + np.log(1 / (1 + Rbc)):ri_real + 0', data)
...
def initialize_ensemble(
    ...
    for (prefix,output_filenames) in zip(['training','testing','validation'],output_filenames_sets):
        ...
        save_ensemble(ensemble_dir, prefix, df, output_filenames, [])
    
def read_ensemble(ensemble_dir, prefix):
    path_df, path_filenames, path_completed_filenames = get_ensemble_paths(ensemble_dir, prefix)
...
def save_ensemble(
        ensemble_dir, prefix,
        df, output_filenames, completed_filenames):
...
def build_ensemble_dataframe(
    ...
    df, output_filenames, completed_filenames = read_ensemble(ensemble_dir, prefix)
...
def append_ensemble_dataframe(
        df,output_filename,wvls,rhs,varnames,morphology='core-shell'):
    data = dataframe_to_dict(df)
    optical_population = read_optical_population(output_filename,morphology=morphology)
    for rh in rhs:
        for wvl in wvls:
            for varname in varnames:
                # if varname == 'Edry_minus1':
                #     Edry = get_population_variable(optical_population, 'Eabs_clear', wvl=wvl, rh=0.)
                #     data[varname].append(Edry - 1.)
                # elif varname == 'Ewet_over_dry':
                #     Eclear = get_population_variable(optical_population, 'Eabs_clear', wvl=wvl, rh=rh)
                #     Edry = get_population_variable(optical_population, 'Eabs_clear', wvl=wvl, rh=0.)
                #     data[varname].append(Eclear/Edry)
                # elif varname == 'Ewet_over_dry_minus1':
                #     Eclear = get_population_variable(optical_population, 'Eabs_clear', wvl=wvl, rh=rh)
                #     Edry = get_population_variable(optical_population, 'Eabs_clear', wvl=wvl, rh=0.)
                #     data[varname].append(Eclear/Edry - 1.)
                # elif varname == 'dry_shell_real_ri':
                #     data[varname].append(get_population_variable(optical_population, 'shell_real_ri', wvl=wvl, rh=0.))
                # elif varname == 'dry_shell_imag_ri':
                #     data[varname].append(get_population_variable(optical_population, 'shell_imag_ri', wvl=wvl, rh=0.))
                # else:
                list(data[varname]).append(get_population_variable(optical_population, 'Rbc', wvl=wvl, rh=rh))
            ...
            # Eclear = get_population_variable(optical_population, 'Eabs_clear', wvl=wvl, rh=rh)
            # data['Ewet_over_dry'] = Eclear/Edry
            # data['Ewet_over_dry_minus1'].append(Eclear/Edry-1.)
    df = pd.DataFrame(data)
    return df
...
#         optical_population = read_optical_population(output_filename,morphology=morphology)
#         data['filenames'].append(output_filename)
#         data['Rbc'].append(get_population_variable(optical_population, 'Rbc', wvl=wvl, rh=0.))
#         data['kap'].append(get_population_variable(optical_population, 'shell_tkappa', wvl=wvl, rh=0.))
#         data['ri_real'].append(get_population_variable(optical_population, 'real_shell_ri', wvl=wvl, rh=0.))
#         data['ri_imag'].append(get_population_variable(optical_population, 'imag_shell_ri', wvl=wvl, rh=0.))
...
def get_population_variable(optical_population, varname, wvl=350e-9, rh=0.):
    if varname == 'rh' or varname == 'RH':
        ...
        idx_shell, = np.where(
            [one_idx not in [optical_population.idx_bc()]
        ...
        vardat = (optical_population.shell_dry_vols+optical_population.h2o_vols[idx_rh])/optical_population.core_vols
        ...
        vardat = optical_population.get_average_ri(
            morph_component='shell', ri_component='imag', rh=0., wvl=wvl)
    elif varname == 'shell_real_ri_dry':
        vardat = optical_population.get_average_ri(
            morph_component='shell', ri_component='real', rh=0., wvl=wvl)
        ...
        babs_total = optical_population.get_optical_coeff(optics_type='total_abs',rh=0.,wvl=float(wvl))
        babs_pure_bc = optical_population.get_optical_coeff(optics_type='pure_bc_abs',rh=0.,wvl=float(wvl))
        vardat = babs_total/babs_pure_bc
