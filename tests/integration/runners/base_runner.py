...
# Load checks module by file path to avoid package import issues when pytest
# imports tests as top-level modules.
HERE = pathlib.Path(__file__).resolve().parent.parent
spec = importlib.util.spec_from_file_location("tests.integration.checks", HERE / "checks.py")
checks = importlib.util.module_from_spec(spec)
spec.loader.exec_module(checks)

assert_no_nans = checks.assert_no_nans
assert_nonnegative = checks.assert_nonnegative
assert_monotonic_increasing = checks.assert_monotonic_increasing
load_expected_csv = checks.load_expected_csv
write_diff_csv = checks.write_diff_csv
DEFAULT_RTOL = checks.DEFAULT_RTOL
DEFAULT_ATOL = checks.DEFAULT_ATOL
