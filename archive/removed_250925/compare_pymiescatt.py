...
base_pop_cfg = cfg["population"]
var_cfg = cfg[varname]
plot_one_line(varname='b_scat', base_config_path=base_config_path)
wvl_nm, b_scat_m, b_abs_m = pymiescatt_lognormal_b_scat(pop_cfg=base_pop_cfg, optics_cfg=var_cfg)
plt.plot(wvl_nm*1e-9, b_scat_m, 'k--', label='PyMieScatt lognormal integral')
plt.xlabel('Wavelength (m)')
plt.ylabel('b_scat (m^-1)')
repo_root = Path(__file__).resolve().parent.parent
out = repo_root / "examples" / "b_scat_lognormal_vs_pymiescatt.png"
