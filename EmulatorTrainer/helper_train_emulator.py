...
def train_Eabs_clear_dry_onewvl(df_training, num_tune=1000, num_samples=3000):
    ...
    model = bambi.Model('Eclear_dry_minus1 ~ np.log(1 + Rbc_dry) + np.log(1 + Rbc_dry):shell_real_ri_dry_bc + 0', data=df_training)
    ...
    Edry_fun = (
        lambda Rbc, ri_real: model_params['np.log(1 + Rbc_dry)']*np.log(1/(1+Rbc)) 
        + model_params['np.log(1 + Rbc_dry):shell_real_ri_dry_bc']*np.log(1/(1+Rbc))*ri_real + 1.)
